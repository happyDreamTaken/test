...
class CaptchaRequest {
    ...
    class func captchaAccept(_ type: CaptchaType ,success:@escaping (CaptchaResponseData) ->(), failure:@escaping (Error) ->()) {
        let url = "/api/captcha/get_captcha"
        var s = "blockPuzzle";
        switch type {
        ...
        IvyGateServerAPI.shared.getSmsImages(type: 0){
            (json , error) in
            
            guard let maindata = json else {
                return
            }
            
            let res = maindata["data"]["repData"]
    ...
    class func captchaCheck(_ type: CaptchaType, pointJson: String = "", token: String = "",success:@escaping (CaptchaResponseData) ->(), failure:@escaping (Error) ->()) {
        let url = "/api/captcha/check_captcha"
        var s = "blockPuzzle";
        switch type {
        case CaptchaType.puzzle:
            s = "blockPuzzle";
        ...
        IvyGateServerAPI.shared.checkSmsImages(type: 0, pointJson: pointJson, token: token){(response , error) in
            
            guard let maindata = response else {
                    return
            }
