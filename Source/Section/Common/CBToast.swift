...
public class CBToast: NSObject {
    
    //显示菊花
   public class func showToastAction() {
    ...
    //隐藏菊花
   public class func hiddenToastAction() {
        if toastView != nil {
            let indicatorView = toastView?.viewWithTag(10) as! UIActivityIndicatorView
    ...
    //默认显示消息-->center
  public  class func showToastAction(message : NSString) {
        self.showToast(message: message, aLocationStr: "center", aShowTime: 2.0)
    }
 
    //显示消息
   public class func showToast(message : NSString?, aLocationStr : NSString?, aShowTime : TimeInterval) {
   ...
   public class func showIndicatorToastAction(message : NSString) {
        self.showIndicatorToast(message: message, aLocationStr: "center", aShowTime: 2.0)
    }
    
    
    //显示(带菊花的消息)
   public class func showIndicatorToast(message : NSString?, aLocationStr : NSString?, aShowTime : TimeInterval) {
        if Thread.current.isMainThread {
    ...
    //隐藏(带菊花的消息)
   public class func hiddenIndicatorToastAction() {
...
//MARK: init UI
public extension CBToast {
    
    static var toastView : UIView?
   public class func currentToastView() -> UIView {
   ...
   public class func currentToastLabel() -> UILabel {
        objc_sync_enter(self)
        if toastLabel == nil {
   ...
   public class func currentToastViewLabel() -> UIView {
        objc_sync_enter(self)
        if toastViewLabel == nil {
...
public extension CBToast {
    ...
    //根据字符串长度获取对应的宽度或者高度
   public class func stringText(aText : NSString?, aFont : CGFloat, isHeightFixed : Bool, fixedValue : CGFloat) -> CGFloat {
